...
const InformacionRegistroSchema = new Schema({
  ...
  encuestador: { type: Schema.Types.ObjectId, ref: "users", required: true },
});
...
const InformacionUbicacionSchema = new Schema({
  posesionTimeNumber: {
    type: Number,
    required: true,
    description: "VALOR NUMERICO DE TIEMPO DE POSESIÓN",
  },
  posesionTimeUnit: {
    type: String,
    required: true,
    description: "UNIDAD DE TIEMPO DE POSESIÓN",
  },
  sector: {
    type: String,
    required: true,
  ...
  familyCount: {
    type: Number,
    required: true,
    description: "NÚMERO DE FAMILIAS EN EL LOTE",
  },
  peopleCount: {
    type: Number,
    required: true,
    description: "PERSONAS QUE HAY EN EL LOTE",
  },
  houseState: {
    type: String,
    required: true,
    description: "TIPO DE CONTRUCCION DE LA CASA",
  },
});

// Aseguramos que la ubicación sea única a través de un índice compuesto
InformacionUbicacionSchema.index(
  { sector: 1, barrio: 1, manzana: 1, lotenumero: 1 },
  { unique: true }
);
...
RegistroSchema.index(
  {
    "informacionUbicacion.sector": 1,
    "informacionUbicacion.barrio": 1,
    "informacionUbicacion.manzana": 1,
    "informacionUbicacion.lotenumero": 1,
  },
  { unique: true }
);
...
//control de acceso a la ruta
RegistroSchema.statics.isProtected = function (method) {
  const protectedMethods = ["get"]; // método 'post' libre
  return protectedMethods.includes(method);
};
